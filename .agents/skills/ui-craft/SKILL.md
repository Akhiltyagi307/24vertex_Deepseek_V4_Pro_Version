...
For **Supabase Studio–shaped product UI** (EduAI, dark-first dashboards, SQL/editor surfaces, data tables), **[design-v2.md](references/design-v2.md)** is the **canonical spec** — tokens, typography, components, motion, charts, and a11y. Every other file under `references/` **defers to design-v2** on conflicts and only **supplements** it (general craft, OKLCH theory, marketing landings, animation deep-dives). When that product context applies, **read design-v2 before** color, typography, layout, dashboard, etc. Use **[inspiration.md](references/inspiration.md)** for SaaS **marketing** patterns that intentionally differ from Studio chrome.
...
| Project design system / brand / tokens | Read [design-v2.md](references/design-v2.md) |
...
- **Reference files are CSS-first.** Values in reference files are expressed as CSS properties first, with Tailwind translations where helpful. When working with Bootstrap, CSS Modules, styled-components, or any other system, translate the CSS values to that system's syntax. For Studio-shaped product UI, **design-v2.md** is authoritative; other references supplement it.
...
Deep dives for specialized work. Read only what's relevant to the task at hand. **Supabase / EduAI product UI:** start with [design-v2.md](references/design-v2.md); other references align with it (see [Reference hierarchy](#reference-hierarchy) under Routing).

| Reference | When to Read |
|-----------|-------------|
| [design-v2.md](references/design-v2.md) | **Canonical** Supabase Design System v2 — colors, type, spacing, components, SQL editor, charts, toasts, Tailwind §19. Read first for Studio-shaped apps. |
