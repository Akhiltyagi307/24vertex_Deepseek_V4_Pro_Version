...
> **Supabase Studio / EduAI defaults:** [**design-v2.md**](./design-v2.md) §16 — motion is **functional**, typically **under 200ms** for micro-interactions; **60ms** hover backgrounds, **120ms** dropdowns, **150–200ms** modals/sheets, **200ms** sidebar width, **500ms** progress bars; **ease-out** for enter, **ease-in** for exit; **do not animate** `width`/`height`/`top`/`left` — use `transform` + `opacity`. Align orchestration below with those budgets when building Studio-shaped UI.
