...
> **Canonical product UI (Supabase Studio / EduAI):** [**design-v2.md**](./design-v2.md) §3 — **13px** default body; **Inter-class** sans stack; **JetBrains Mono** for UUIDs, SQL, metrics, and uppercase data labels (`letter-spacing: 0.08em`, mono). Full scale table lives there.
...
### Scale — Supabase / EduAI (design-v2)

| Style | Size | Weight | Notes |
|-------|------|--------|--------|
| Display | 36px | 700 | Marketing / hero only |
| H1 | 24px | 600 | Page titles |
| H2 | 18px | 600 | Sections |
| H3 | 15px | 600 | Card titles |
| H4 | 13px | 600 | Group labels |
| Body | **13px** | 400 | Default UI copy |
| Body-sm | 12px | 400 | Help text |
| Caption | 11px | 400 | Meta |
| Label-upper | 11px | 500 | Mono, uppercase, +0.08em — metric / column labels |
| Code-inline / block | 12–13px | 400 | JetBrains Mono |
| Numeric-hero | 24px | 600 | Stat values in metric cards |

**Dashboard density:** design-v2 caps **font-weight at 600** for product UI (avoid 700+ except display/marketing) — see design-v2 §21.

### Scale (generic / marketing-heavy apps)

When not following Studio density, a common rem scale:
```css
...
- **Body fonts** for text — readable, neutral, well-hinted (Studio: Inter-close sans — design-v2 §3.1)
- **Monospace** as intentional accent — Studio mandates **JetBrains Mono** for all data/SQL/code values (design-v2 §3.1, §8.7)
...
- **Body text (14-18px)**: leave `letter-spacing` alone — the font designer optimized it (Studio body is **13px** — still typically `0` tracking — design-v2 §3.2)
- **Small text/labels (11-13px)**: slight positive tracking `+0.01em` to `+0.02em` for readability
- **ALL CAPS labels**: needs `+0.05em` to `+0.1em` tracking; **Studio metric-style labels use +0.08em + mono** (design-v2 §3.2)
...
- **Studio tables:** full grid uses **JetBrains Mono** at **12px**; `NULL` → faint/muted foreground; UUIDs smaller/muted — design-v2 §8.7, §3.3
