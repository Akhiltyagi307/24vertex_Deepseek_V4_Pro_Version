...
> When reviewing **Supabase Studio / EduAI** interfaces, cross-check [**design-v2.md**](./design-v2.md) §21 (do/don’t): tokens vs raw hex, **border** elevation on dark cards, **one green** accent, **JetBrains Mono** for data, **`foreground-muted`** not for primary reading, focus rings **`:focus-visible`**.
...
### Design Audit
- [ ] Layered shadows (ambient + direct) **or** Studio rule: **borders** for card elevation on dark backgrounds (design-v2 §1)
- [ ] Nested radii: child ≤ parent
- [ ] Hue-consistent borders/shadows on colored backgrounds
- [ ] APCA contrast met; **Studio:** verify `foreground-muted` only for non-critical captions (design-v2 §7.2)
...
- [ ] **Studio:** single green accent discipline; data in **mono**; no **700+** weights in dashboard UI (design-v2 §21)
