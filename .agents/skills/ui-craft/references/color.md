...
> **Canonical product UI (Supabase Studio / EduAI):** [**design-v2.md**](./design-v2.md) §2, §15, §18 — brand green `#3ECF8E`, dark-first backgrounds (`#1C1C1C` … `#333333` scale), semantic status colors, and full CSS variable names. Prefer those tokens in app chrome; use this file for general OKLCH craft and non–Studio surfaces.
...
**Supabase / EduAI (design-v2):** Accent is **one electric green** (`#3ECF8E`); amber = warning/production; red = destructive; blue = informational — see design-v2 §2.5. **Elevation = 1px borders and background steps**, not drop shadows on cards in dark mode (design-v2 §1, §21).
...
- Pure black (`#000`) or pure white (`#fff`) for **page** backgrounds or body text (design-v2 §21); exception: **text on primary CTA** may be `#000` on `#3ECF8E` for contrast
...
**Studio-shaped apps:** map names to design-v2 variables — e.g. `--foreground-default`, `--border-default`, `--brand`, `--destructive`, `--warning`, `--info` (full list in design-v2 §18). Below is a **generic** token shape for other products:
...
- **Limit accent color to one per view** (Studio: green only as accent; status hues are semantic, not decorative — design-v2 §21)
- Use existing theme tokens before introducing new ones; **never raw hex in components** when design-v2 variables exist (§21)
- Never gradients unless explicitly requested
- Never glow effects as primary affordances
- Layered shadows: ambient + direct light — **except** Supabase-style dark cards: prefer **border** elevation over card `box-shadow` (design-v2 §1)
- Crisp edges: semi-transparent borders + shadows (where shadows are appropriate)
