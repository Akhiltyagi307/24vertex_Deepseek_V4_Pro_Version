...
if (!process.env.NEXT_PUBLIC_APP_URL?.trim()) {
	process.env.NEXT_PUBLIC_APP_URL = "https://app.24vertex.example.com";
}
...
function buildReportReadyStudent(): Sample {
	...
	const html = renderEmailShell({
		...
		secondaryCta: { label: "View in 24Vertex", href: PORTAL_REPORT_URL },
	});
	return {
...
<title>24Vertex email previews</title>
</head>
...
<main style="max-width:760px;margin:48px auto;padding:0 24px;">
  <div style="display:inline-block;padding:6px 12px;border-radius:999px;background:#e6f4ee;color:#1f6f4f;font-size:11px;font-weight:600;letter-spacing:0.06em;text-transform:uppercase;">24Vertex</div>
