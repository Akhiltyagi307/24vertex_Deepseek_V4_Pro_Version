-- Ensure profiles SELECT policies are recursion-safe.
-- Some environments can still carry an old teacher-era policy that queried
-- public.profiles inside a profiles policy expression, which triggers 42P17.
...
-- Keep schema clean if older recursion workaround function is still present.
