-- Atomic coupon redemption to prevent partial writes and race-condition over-redemption.
-- This function performs validation + entitlement grant + redemption write in one transaction.
...
BEGIN
	...
	SELECT id, is_active, expires_at, max_redemptions, redemptions_count
	INTO v_coupon
	FROM public.coupons
...
GRANT EXECUTE ON FUNCTION public.billing_redeem_coupon_atomic(UUID, UUID, VARCHAR, INTEGER, INTEGER, INTEGER) TO service_role;
