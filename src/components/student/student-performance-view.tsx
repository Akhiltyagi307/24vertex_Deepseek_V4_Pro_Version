...
import { PerformanceTopicSheet } from "@/components/student/performance/performance-topic-sheet";
import {
	emptyPerformanceMatrixMessage,
	formatLastTest,
	formatScore,
	performanceDetailSurfaceClass,
	performanceStatusBadgeClass,
	practiceHref,
	rowMatchesTopicSearch,
	statusBadgeVariant,
	trendLabel,
} from "@/components/student/performance/performance-view-helpers";
...
export function StudentPerformanceView({
	...
	return (
		<div className="flex w-full min-w-0 flex-col gap-6 py-6 pb-24 medium:gap-7 medium:py-8 medium:pb-28">
			...
			{detailSubjectId ? (
				<PerformanceTopicSheet
					open={sheetOpen}
					onOpenChange={(o) => {
						if (!o) perfDispatch({ type: "close_sheet" });
					}}
					sheetRow={sheetRow}
					allowPractice={allowPractice}
					practiceOpts={practiceOpts}
				/>
			) : null}
		</div>
	);
