...
/**
 * GET /api/parent/notifications?cursor=&filter=all|unread&limit=
 */
export async function GET(request: Request) {
	...
	try {
		const [page, unreadCount] = await Promise.all([
			listNotificationsForRecipient(supabase, {
				userId: user.id,
		...
		logSupabaseError("parent.notifications.list", err as { message?: string }, {
			userId: user.id,
		});
		return NextResponse.json(
