...
import { resumeSubscriptionForProfile } from "@/lib/billing/subscription-lifecycle-ops";
import { db } from "@/db";
...
export async function POST(request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
	return Sentry.withScope(async (scope) => {
		...
		const rl = await consumeAdminActionRateLimit({
			action: ADMIN_ACTIONS.SUBSCRIPTION_RESUME,
		...
		const result = await resumeSubscriptionForProfile(sub.profileId);
		if (!result.ok) {
