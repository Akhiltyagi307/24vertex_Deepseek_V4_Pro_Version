...
import { fetchTeacherClassPerformanceSummary } from "./teacher-dashboard-actions";
...
export function TeacherDashboardPanel({
	...
	const [classPerformanceSummary, setClassPerformanceSummary] = useState<TeacherClassPerformanceSummary | null>(null);
	const [classPerformanceError, setClassPerformanceError] = useState<string | null>(null);
	const [classPerformancePending, setClassPerformancePending] = useState(true);
	...
	useEffect(() => {
		let cancelled = false;
		setClassPerformancePending(true);
		setClassPerformanceError(null);
		setClassPerformanceSummary(null);
		void (async () => {
			try {
				const res = await fetchTeacherClassPerformanceSummary({ grade, section, subjectId });
				if (cancelled) return;
				if ("error" in res) {
					setClassPerformanceError(res.error);
					setClassPerformanceSummary(null);
					return;
				}
				...
				if (cancelled) return;
				setClassPerformanceError("Could not load class performance for this scope.");
				setClassPerformanceSummary(null);
			} finally {
				if (!cancelled) {
					setClassPerformancePending(false);
				}
			}
		})();
		return () => {
	...
	return (
		<div className="w-full min-w-0 space-y-8 py-6">
			...
			<TeacherDashboardPerformanceBandStrip
				...
				pending={classPerformancePending}
