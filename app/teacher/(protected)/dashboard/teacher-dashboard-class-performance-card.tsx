...
function UpliftTopicRow({ opportunity }: { opportunity: TeacherClassUpliftOpportunity }) {
	return (
		<li className="border-t border-border/50 pt-3 first:border-t-0 first:pt-0">
			<p className="line-clamp-2 text-base font-medium leading-snug text-foreground">{opportunity.topicName}</p>
			...
			<p className="mt-2 text-muted-foreground text-sm leading-normal">
				Average {Math.round(opportunity.averagePercent)}% across {pluralize(opportunity.studentsTested, "student")};{" "}
		...
		<div className="mt-5 grid gap-5 medium:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]" aria-hidden>
			...
			<div className="space-y-3 rounded-lg border border-border/70 bg-muted/20 p-4">
				...
				{Array.from({ length: 3 }).map((_, index) => (
					<div key={index} className="space-y-2 border-t border-border/50 pt-3 first:border-t-0 first:pt-0">
...
export function TeacherDashboardClassPerformanceCard({
	...
	const upliftOpportunities = summary?.upliftOpportunities ?? [];
		...
		>
			...
			{pending && summary == null ? (
					...
					<p className="text-muted-foreground text-sm leading-normal">
						{activeOrganizationName
							...
							: linkedStudentCount > 0
								...
								: "Link students first, then this card will show the recent class average and the weakest topics to revisit."}
					</p>
				...
				<div className="mt-5 grid min-h-0 flex-1 gap-5 medium:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
					...
					<div className="flex min-h-0 min-w-0 flex-col rounded-lg border border-border/70 bg-muted/20 p-4">
						<div className="flex shrink-0 items-start gap-2">
							...
							<div className="min-w-0">
								...
								<p className="mt-0.5 text-muted-foreground text-xs">Top 5 weakest topics in this scope</p>
							</div>
						</div>
						{upliftOpportunities.length > 0 ? (
							<>
								<ul
									className="mt-3 min-h-0 flex-1 space-y-0 overflow-y-auto overscroll-y-contain pr-1 [-webkit-overflow-scrolling:touch]"
									aria-label="Weakest topics for score uplift"
								>
									{upliftOpportunities.map((opportunity) => (
										<UpliftTopicRow key={opportunity.topicId} opportunity={opportunity} />
									))}
								</ul>
								<p className="mt-3 shrink-0 text-muted-foreground text-xs leading-normal">
									Use these topics for the next review block or targeted practice set.
								</p>
							</>
						) : (
							<p className="mt-3 text-muted-foreground text-sm leading-normal">
								There is not enough tested topic data in this scope to recommend focused review areas yet.
							</p>
						)}
					</div>
				</div>
