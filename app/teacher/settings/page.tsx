...
export default async function TeacherSettingsPage() {
	...
	let orgStudentRoster: {
		initialRows: Awaited<ReturnType<typeof listOrganizationStudentsWithFilters>>;
		filterOptions: Awaited<ReturnType<typeof getOrganizationRosterFilterOptions>>;
	} | null = null;
	if (activeOrganization) {
		const [initialRows, filterOptions] = await Promise.all([
			listOrganizationStudentsWithFilters({ organizationId: activeOrganization.id }),
			getOrganizationRosterFilterOptions(activeOrganization.id),
		]);
		orgStudentRoster = { initialRows, filterOptions };
	}
	...
	return (
		<TeacherAccountSettingsForm
			...
			orgStudentRoster={orgStudentRoster}
