		...
		<div className="space-y-6">
			...
			{rows.length === 0 ? (
				...
				<div className="overflow-hidden rounded-xl border border-border/80 shadow-sm">
					<div className="grid grid-cols-[minmax(0,1fr)_auto_auto_auto] gap-3 border-b border-border bg-muted/40 px-4 py-2 text-muted-foreground text-xs font-medium uppercase tracking-wide">
						<span>Name</span>
					...
					<ul className="divide-y divide-border/80">
						{rows.map((row) => (
							...
							>
								...
								<span className="font-mono text-muted-foreground text-xs tabular-nums sm:text-left">
									{row.studentLinkCode ?? "—"}
								</span>
							</li>
						))}
